...
// internal system attributes (int memory)
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector) {
    ...
    if (diskstream_seek(stream, directory_start_pos) != STATUS_OK) {
        res = -EIO;
        goto out;
    }
    while (1) {
        if (diskstream_read(stream, &item, sizeof(item)) != STATUS_OK) {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory) {
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstream_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != STATUS_OK) {
        res = -EIO;
        goto out;
    }

    if (diskstream_read(stream, dir, root_dir_size) != STATUS_OK) {
        res = -EIO;
        goto out;
    }
...
fat16_resolve(struct disk *disk) {
    ...
    // load header
    if (diskstream_read(stream, &fat_private->header, sizeof(fat_private->header)) != STATUS_OK) {
        res = -EIO;
        goto out;
    }
...
// parse a filename to remove spaces from the end of a fat filename
...
void fat16_get_full_relative_filename(struct fat_directory_item *item, char *out, int max_len) {
    ...
    if (item->ext[0] != 0x00 && item->ext[0] != 0x20) { // file has an extension
        *out_tmp++ = '.';
...
static int fat16_get_fat_entry(struct disk *disk, int cluster) {
    ...
    res = diskstream_seek(stream, fat_table_position * (cluster * FAT16_FAT_ENTRY_SIZE));
    if (res < 0) 
        goto out;
